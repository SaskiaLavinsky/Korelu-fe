...
export default function TextCorrectionPage() {
  ...
  // ====== STYLES (samakan tampilan ketiganya) ======
  const TA_BASE =
    ...
    for (const [asal, kandidat] of Object.entries(symspellCandidates)) {
      if (Array.isArray(kandidat) && kandidat.length > 0) {
        const parts = kandidat.map(([term, jw, pll]) => {
          const jwStr =
            jw !== null && jw !== undefined ? Number(jw).toFixed(2) : "-";
          const pllStr =
            pll !== null && pll !== undefined ? Number(pll).toFixed(3) : "-";
  ...
  const handleTextCorrection = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/koreksi`, {
        ...
        body: JSON.stringify({ kalimat: inputText }),
      });
      if (!res.ok) throw new Error("Gagal menghubungi server.");
      const data = await res.json();

      setHasilTeks(data.teks || "");
      setHasilTokens(data.tokens || []);
      setUbahIndex(data.ubah_index || []);
      setIdxSingkatan(data.idx_singkatan || []);
      setIdxPolitikFix(data.idx_politik_fix || []);
      setIdxTypoFix(data.idx_typo_fix || []);
        ...
        <div className="bg-white shadow-md rounded-xl p-6 mb-8">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            {!isLoading && processTimeMs !== null && (
              <span className="text-sm text-gray-600" aria-live="polite">
                ...
                <span className="font-semibold">
                  {(() => {
                    const totalSeconds = Math.floor(processTimeMs / 1000);
            ...
            <div className="text-xs text-gray-600 flex flex-wrap gap-2">
              ...
              <span className="px-2 py-0.5 rounded bg-pink-100">
                Perbaikan Kamus Politik
              </span>
              <span className="px-2 py-0.5 rounded bg-yellow-100">
                Perbaikan Typo umum
              </span>
            </div>
          </div>
          ...
          {/* ======================
              1) HASIL KOREKSI (textarea-like, tetap pakai DIV agar highlight bisa tampil)
             ====================== */}
          <div className="mt-3">
            ...
            <div className={`${TA_BASE} select-none`}>
              {isLoading ? (
                <p className="text-gray-500 ">
                  Sedang memproses koreksi. Mohon tunggu...
                </p>
              ) : hasilTokens.length ? (
            ...
            {/* ======================
                2) HASIL AKHIR (textarea asli, tampilan selaras)
               ====================== */}
          ...
          {/* ======================
              3) KANDIDAT (JW → PLL) — textarea asli, tampilan selaras
             ====================== */}
